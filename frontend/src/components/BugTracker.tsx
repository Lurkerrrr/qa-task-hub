            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                ...
                <div className="flex gap-3 w-full md:w-auto">
                    ...
                    <button
                        ...
                        className="bg-blue-600 text-white px-4 py-2 rounded-lg font-medium hover:bg-blue-700 transition flex items-center gap-2 shadow-md transform active:scale-95"
                    >
                        {t.btn_add}
                    </button>
                </div>
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                            <div className="flex-grow">
                                <div className="flex flex-wrap items-center gap-3 mb-2">
                                    ...
                                    <span className="text-xs bg-slate-100 text-slate-700 px-2 py-0.5 rounded flex items-center gap-1">
                                        👤 {bug.assignee}
                                    </span>
                                </div>
                            ...
                            <div className="flex flex-row md:flex-col items-center md:items-end gap-3 w-full md:w-auto">
                                <select
                                    className={`text-sm border rounded-lg px-3 py-1.5 outline-none font-medium ${bug.status === 'Done' ? 'bg-green-50 text-green-700' : bug.status === 'In Progress' ? 'bg-purple-50 text-purple-700' : 'bg-white'}`}
                                    value={bug.status}
                                    ...
                                    <div className="p-6">
                                        <form onSubmit={handleSubmit} className="space-y-5">
                                            <div>
                                                ...
                                                <input
                                                    type="text"
                                                    className={`w-full p-3 border rounded-lg outline-none ${errors.title ? 'border-red-500' : 'border-gray-200 focus:ring-blue-500 focus:border-blue-500'}`}
                                                    value={newBug}
                                            ...
                                            <div>
                                                ...
                                                <textarea
                                                    className="w-full p-3 border border-gray-200 rounded-lg h-32 resize-none outline-none focus:ring-blue-500 focus:border-blue-500"
                                                    value={steps}
                                            ...
                                            <div className="flex justify-end gap-3">
                                                <button type="button" onClick={handleCloseModal} className="px-5 py-2.5 font-medium text-gray-600 hover:bg-gray-100 rounded-lg">{t.btn_cancel}</button>
                                                <button type="submit" className="bg-blue-600 text-white px-8 py-2.5 rounded-lg font-bold hover:bg-blue-700 shadow-lg shadow-blue-200 transform active:scale-95">{t.btn_add}</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
