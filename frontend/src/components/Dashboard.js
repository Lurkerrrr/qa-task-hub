...
const Dashboard = ({ t, bugs = [] }) => {
    ...
    const activeCriticalBugs = safeBugs.filter(
        (b) => (b.priority === 'Highest' || b.severity === 'Critical') && b.status !== 'Done'
    ).length;
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-10">
            <div className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">
                        {new Date().toLocaleDateString()} • {t.active_tasks}:{' '}
                        <span className="font-bold text-blue-600">{activeBugs}</span>
                    </p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <motion.div
                    whileHover={{ y: -5 }}
                    className="bg-white p-6 rounded-2xl shadow-sm border-l-4 border-red-500"
                >
                    <p className="text-gray-400 text-sm font-medium uppercase">{t.critical}</p>
                    <h3
                        className={`text-4xl font-bold mt-2 ${activeCriticalBugs > 0 ? 'text-red-600' : 'text-gray-800'}`}
                    >
                        {activeCriticalBugs}
                    </h3>
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        <div className="flex justify-between mb-2">
                            ...
                            <span
                                className={`text-sm font-bold ${criticalDensity >= 80 ? 'text-red-600' : 'text-gray-600'}`}
                            >
                                {criticalDensity}%
                            </span>
                        </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                ...
                <div className="space-y-4">
                    {recentActivity.map((bug) => (
                        <div
                            key={bug.id}
                            className="flex items-center gap-3 p-3 hover:bg-gray-50 rounded-lg transition border-b border-gray-50 last:border-0"
                        >
                            <div
                                className={`w-2 h-2 rounded-full flex-shrink-0 ${
                                    bug.priority === 'Highest' || bug.priority === 'Critical'
                                        ? 'bg-red-500'
                                        : bug.priority === 'High'
                                          ? 'bg-orange-500'
                                          : 'bg-green-500'
