...
const ExternalAPI = ({ t }) => {
    const [resource, setResource] = useState('users'); // users, posts, comments
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(fullUrl + '?_limit=5'); // Берем только 5 штук
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-4">
                    <div className="flex gap-2">
                        {['users', 'posts', 'todos', 'photos'].map((res) => (
                            <button
                                key={res}
                                onClick={() => setResource(res)}
                                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${resource === res
                                        ? 'bg-blue-600 text-white shadow-md'
                                        : 'bg-slate-100 text-gray-600 hover:bg-slate-200'
                                    }`}
                            >
                                Get {res.charAt(0).toUpperCase() + res.slice(1)}
                            </button>
                        ))}
                    </div>
                </div>

                {/* Строка запроса (Terminal Style) */}
                <div className="bg-slate-900 rounded-lg p-4 font-mono text-sm overflow-x-auto">
                    <div className="flex flex-col md:flex-row md:items-center gap-4 text-gray-300">
                        ...
                        <div className="flex items-center gap-2 flex-grow">
                            ...
                            <span className="text-blue-400 break-all">{fullUrl}</span>
                        </div>

                        <div className="flex items-center gap-4">
                            <div>
                                <span className="text-gray-500 mr-2">{t.status}:</span>
                                <span className={`${status === 200 ? 'text-green-400' : 'text-red-400'}`}>{status || '...'}</span>
                            </div>
                            <div>
